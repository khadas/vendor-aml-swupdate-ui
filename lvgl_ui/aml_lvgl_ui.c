 ...
 */
void lv_bar_refresh(lv_obj_t *bar, int32_t data)
{
    lv_event_send(bar, LV_EVENT_REFRESH, &data);
}
...
int swupdateui_run(int argc, char *argv[])
{
    ...
    sleep(1);
    ref_ent.bar = lv_bar_create(lv_scr_act());
    ref_ent.label = lv_label_create(lv_scr_act());
    ref_ent.fd = sock_conn();
    ref_ent.p_bar_refresh = lv_bar_refresh;

    /*Init the lable*/
    lv_label_init(ref_ent.label);

    /* Init bar config */
    lv_bar_init(ref_ent.bar, &ref_ent.style_bg);

    /*Init the image*/
    lv_img_init(&ref_ent.img, file_path);
